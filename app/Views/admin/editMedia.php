...
<form method="POST" action="<?= URL_ROOT_PUBLIC ?>/index.php?url=admin/editMedia">
...
<form method="POST" action="<?= URL_ROOT_PUBLIC ?>/index.php?url=admin/deleteMedia">
    <input type="hidden" name="idMedia" value="<?= $media->getIdMedia() ?>">
    <button type="submit" class="btn btn-danger mt-3" onclick="return confirm('Supprimer ce média ?')">Supprimer</button>
</form>

<a href="<?= URL_ROOT_PUBLIC ?>/index.php?url=admin/dashboard" class="btn btn-secondary mt-3">Retour</a>